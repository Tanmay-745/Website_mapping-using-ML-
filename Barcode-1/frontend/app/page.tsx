...
import { Upload, Trash2, RotateCcw, Edit2, Download } from "lucide-react"
...
export default function BarcodePage() {
  ...
  const downloadAllCSV = () => {
    const headers = ["Barcode", "Status", "LAN", "Lender Name", "Customer Name/Phone", "Created AT", "Used Date"];
    ...
    const link = document.createElement("a");
    link.setAttribute("href", url);
    link.setAttribute("download", `all_barcodes_${new Date().toISOString().split('T')[0]}.csv`);
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
