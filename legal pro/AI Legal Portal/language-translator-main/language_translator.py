...
def translate():
    ...
    if language_1 == '':
        messagebox.showerror('Language Translator', 'Please fill the Text Box for Translation')
         ...
         # Use the core logic for translation
         global output
         # Map user-friendly language name to code (using a simple reverse lookup for now)
         lang_map = {v: k for k, v in translator_core.get_supported_languages().items()}
         dest_code = lang_map.get(cl, 'en')
         
         output = translator_core.translate_text(language_1, dest_code)
...
# Background settings using Tkinter
