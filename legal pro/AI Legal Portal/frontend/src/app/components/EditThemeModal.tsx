      ...
      <div className="bg-white dark:bg-gray-900 w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[95vh]">
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-8 space-y-8">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-2 relative">
              <label className="text-sm font-medium text-gray-400 absolute left-3 -top-2 bg-white dark:bg-gray-900 px-1 z-10 transition-all">Notice Header (HTML)</label>
              <textarea 
                value={formData.noticeHeaderHtml || ''}
                onChange={e => setFormData({ ...formData, noticeHeaderHtml: e.target.value })}
                className="w-full min-h-[80px] p-4 bg-transparent border border-gray-200 dark:border-gray-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all font-mono text-xs"
                placeholder="<div style='text-align: center'>IMAGE_URL</div>"
              />
              <p className="text-[10px] text-gray-400 pl-1 italic">Optional: Provide an image URL within a div to use as a custom letterhead.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2 relative">
                ...
                <div className="relative">
                  <select 
                    value={formData.advocates[0] || ""}
                    onChange={e => setFormData({ ...formData, advocates: e.target.value ? [e.target.value] : [] })}
                    className="w-full h-14 pl-4 pr-10 bg-transparent border border-gray-200 dark:border-gray-700 rounded-xl appearance-none focus:ring-2 focus:ring-blue-500/20 text-gray-700 font-medium uppercase text-center"
                  >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
               <div className="space-y-2 relative">
                ...
                <div className="relative">
                  <select 
                    value={formData.noticeTypes[0] || ""}
                    onChange={e => setFormData({ ...formData, noticeTypes: e.target.value ? [e.target.value] : [] })}
                    className="w-full h-14 pl-4 pr-10 bg-transparent border border-gray-200 dark:border-gray-700 rounded-xl appearance-none focus:ring-2 focus:ring-blue-500/20 text-gray-700 font-medium uppercase text-center"
                  >
                  ...
                  <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 w-5 h-5 text-blue-600" />
                </div>
              </div>
