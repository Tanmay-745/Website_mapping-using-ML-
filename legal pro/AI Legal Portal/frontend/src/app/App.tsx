...
import { generateNoticeContent, Advocate, Lender, getAdvocates } from "./api";
  ...
  useEffect(() => {
    const fetchAdvocates = async () => {
      try {
        ...
        console.error("Failed to load advocates", e);
      }
    };
    fetchAdvocates();
  }, [activeTab]);
      ...
      <main className="max-w-[1600px] mx-auto px-6 py-4">
        {activeTab === "themes" && (
          <ThemesDashboard 
            ...
            onAddTemplate={(theme) => {
              const advocateName = theme.advocates[0] || "";
              const advocateDetails = advocatesList.find(a => a.name === advocateName);
          ...
          onSave={(updatedTheme) => {
            ...
            if (isUpdate) {
              ...
              if (!hasTemplates && themeWithTemplate.templates[0]) {
                const advocateName = themeWithTemplate.advocates[0] || "";
                const advocateDetails = advocatesList.find(a => a.name === advocateName);
