...
const KNOWN_MAPPINGS: Record<string, string> = {
  ...
  'loan id': 'LAN',
  'bal': 'outstanding amount',
  'amt': 'loan amount',
  'outst': 'outstanding amount',
  'principal': 'loan amount',
  'emi': 'outstanding amount',
  'dob': 'date of birth',
  'disb dt': 'agreement date',
  'disbursement date': 'agreement date',
  'due dt': 'due date',
  'zip': 'pincode',
  'pin': 'pincode',
  'residence': 'address',
...
export function findBestMatch(
  ...
  // 2. Transfer Learning (Learn from user's manual mappings of similar columns)
  for (const [learnedSource, mappedTarget] of Object.entries(learnedMappings)) {
      if (mappedTarget && mappedTarget !== '__SKIPPED__') {
          const sim = calculateSimilarity(sourceHeader, learnedSource);
          if (sim >= 0.85) {
              const targetMatch = targetHeaders.find(t => t.toLowerCase() === mappedTarget.toLowerCase() || t === mappedTarget);
              if (targetMatch) {
                  return { header: targetMatch, confidence: Math.min(sim + 0.1, 1.0) };
              }
          }
      }
  }
...
export function mapHeaders(
  ...
  return sourceHeaders.map((sourceHeader) => {
    ...
    const { header, confidence } = findBestMatch(sourceHeader, targetHeaders, samples, learnedMappings);
